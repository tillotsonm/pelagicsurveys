...
fitted_draws(model_full_tfs,re_formula = NA,scale="response")


expand.grid(volume_c = 0, region = factor(unique(Review_Data_Tows$Review_Region)),year=factor(2002:2020))
...
summary(model_full_tfs)


Review_Data_Tows%>%filter(SurveySeason=="FMWT")%>%
  mutate(Catch = CPUV_Threadfin_Shad_Age_0*Volume)%>%
