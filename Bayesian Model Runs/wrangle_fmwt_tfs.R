...
# Wrangle data -----------------------------------------------------------------
dat_fmwt_tfs_age0 = Review_Data_Tows %>% 
  filter(SurveySeason == 'FMWT') %>% 
  mutate(tfs_age0_catch = CPUV_Threadfin_Shad_Age_0 * Volume,
         tfs_age0_catch = round(tfs_age0_catch),  # correct any minor rounding error
         tfs_age0_catch = as.integer(tfs_age0_catch)) %>% 
  select(tfs_age0_catch, CPUV_Threadfin_Shad_Age_0, 
         Volume, everything()) %>%
  select(-(CPUV_Pacific_Herring_Age_0:Mean_Length_Prickly_Sculpin_Age_0)) %>%  # remove extraneous columns 
  janitor::clean_names() %>% 
  mutate(month = factor(month, levels = 8:12)) %>% 
  rename(lon = station_longitude,
         lat = station_latitude) %>% 
  drop_na(tfs_age0_catch, review_region, region) %>% # Remove rows with NAs in key variables
  arrange(sample_date) %>% 
