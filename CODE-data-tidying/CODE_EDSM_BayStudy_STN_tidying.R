...
LTMR_Raw <- LTMRdata::LTMRpilot()%>%filter(is.na(Length_NA_flag)==T
                                           &Source != "FMWT")%>%
  rename("Gear" = "Method")

#Compile common name taxa list from Five Surveys file

Master_Taxa <- Long_Master %>% distinct (across(CommonName),.keep_all = T)%>%select(CommonName,Genus,Species) %>%
mutate(CommonName = recode(CommonName,
                           "Striped_Bass_Adult" = "Striped_Bass",
...
LTMR_Tidy <- LTMR_Raw %>% uncount(Count)%>%select(-c(Datetime,SampleID,Tow_area,Secchi_estimated,
  ...
  select(SurveySeason,StationCode,Station_Latitude,Station_Longitude,SampleDate,Volume,CommonName,ForkLength,Gear)


Long_Master %>% filter(CommonName =="Starry_Flounder")%>%select(Species)
...
Additional_Surveys%>%filter(Station_Latitude.x!=Station_Latitude.y)%>%
